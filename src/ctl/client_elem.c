...
static void alsactl_client_class_init(ALSACtlClientClass *klass)
{
    ...
    ctl_client_props[CTL_CLIENT_PROP_TYPE] =
        g_param_spec_int("type", "type",
                         ...
                         0,
                         G_PARAM_READABLE);
    g_object_class_install_properties(gobject_class, CTL_CLIENT_PROP_COUNT,
                                      ctl_client_props);
                              ...
                              struct snd_ctl_elem_id *id)
{
    ...
    if (event & SNDRV_CTL_EVENT_MASK_ADD) {
        g_signal_emit(G_OBJECT(client), ctl_client_sigs[CTL_CLIENT_SIG_ADDED],
                      0, id->numid);
    }
    ...
    for (entry = priv->elems; entry; entry = entry->next) {
        ...
        if (event == SNDRV_CTL_EVENT_MASK_REMOVE) {
            priv->elems = g_list_delete_link(priv->elems, entry);
            g_signal_emit_by_name(G_OBJECT(elem), "removed", NULL);
            continue;
        }

        if (event & SNDRV_CTL_EVENT_MASK_VALUE)
            g_signal_emit_by_name(G_OBJECT(elem), "changed", NULL);
        if (event & SNDRV_CTL_EVENT_MASK_INFO)
            g_signal_emit_by_name(G_OBJECT(elem), "updated", NULL);
...
void alsactl_client_listen(ALSACtlClient *self, GError **exception)
{
    ...
    ((CtlClientSource *)src)->tag = g_source_add_unix_fd(src, priv->fd, G_IO_IN);
...
static void ctl_elem_set_property(GObject *obj, guint id, const GValue *val,
                                  GParamSpec *spec)
{
    ALSACtlElem *self = ALSACTL_ELEM(obj);
    ALSACtlElemPrivate *priv = alsactl_elem_get_instance_private(self);
...
static void ctl_elem_finalize(GObject *obj)
{
    ...
    if (!(priv->info.access & SNDRV_CTL_ELEM_ACCESS_OWNER))
        client_ioctl(priv->client, SNDRV_CTL_IOCTL_ELEM_REMOVE, &priv->info.id);
...
static void alsactl_elem_class_init(ALSACtlElemClass *klass)
{
    ...
    ctl_elem_props[CTL_ELEM_PROP_DIMENSION] =
        g_param_spec_boxed("dimension", "dimension",
                           ...
                           G_TYPE_ARRAY,
                           G_PARAM_READABLE);
    g_object_class_install_properties(gobject_class, CTL_ELEM_PROP_COUNT,
                                      ctl_elem_props);
